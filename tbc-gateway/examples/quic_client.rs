...
use std::sync::Arc;
use tbc_engine::transport::{decode_reliable, encode_move_datagram, encode_reliable, WireMessage};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    send.write_all(&encode_reliable(&WireMessage::login())?)
        .await?;
    send.finish()?;
    ...
    move_send
        .write_all(&encode_reliable(&WireMessage::move_intent(
            fwau,
            tick + 1,
            1.0,
            0.0,
    ...
    println!(
        "Reliable move → snapshot tick={:?}",
        move_reply.payload.get("tick")
    );
