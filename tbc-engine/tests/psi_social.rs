...
fn psi_past_shared_npmr_frame() {
  ...
  let donor_fwau = aum.bind_player(2, donor, Vec3::new(10.0, 0.0, 0.0)).unwrap();
...
fn speak_heard_nearby_ai() {
  ...
  let fwau = aum.bind_player(0, iuoc, Vec3::new(-175.0, 0.0, 0.0)).unwrap();
...
fn consent_pact_grants_assist() {
  ...
  let pact = aum.grant_consent(from, target, "assist", 10_000).expect("grant");
