    ...
    impl AumCore {
        ...
        pub fn handoff_fwau(
                ...
                .contains(&to_ruleset)
            {
                return Err(format!(
                    ...
                    self.frames[from_idx].spec.ruleset.id, to_ruleset
                ));
            }
        ...
        pub fn process_shard_handoffs(&mut self) -> Vec<(FwauId, FwauId, usize)> {
            ...
            for i in 0..self.frames.len() {
                let handoffs: Vec<(FwauId, u32)> =
                    self.frames[i].pending_shard_handoffs.drain(..).collect();
                    ...
                    if let Some(to) = to_idx {
                        if to != i {
                            ...
                            if let Some(iuoc_id) = iuoc {
                                self.frames[i].unbind_fwau(fwau);
                                if let Ok(new_fwau) =
                                    self.frames[to].bind_player(&mut self.iuoc, iuoc_id, pos)
                                {
                                    self.rww.publish_fwau_bound(
                                        new_fwau.0,
        ...
        pub fn unbind_death(
            ...
            fwau: FwauId,
            frame_idx: usize,
        ) -> Option<crate::iuoc::ExperiencePacket> {
            let frame_id = self
                .frames
                .get(frame_idx)
                .map(|f| f.spec.id)
