...
use crate::types::{Entity, FrameId, FwauId, IslandId, IuocId, Tick, Vec3};
...
impl Frame {
    ...
    pub fn spawn_demo_world(&mut self, ai_count: usize) {
        for i in 0..ai_count {
            ...
            let e = self
                .world
                .spawn_ai_guy(&format!("AI-Guy-{}", i), pos, i as u64 + 1);
    ...
    pub fn bind_player(
        ...
        self.intent_queues
            .insert(fwau_id, IntentQueue::with_cap(64));
    ...
    pub fn submit_intent(
        ...
        if !self.guardrails.allow_intent(intent.fwau) {
            self.netcode
                .record_reject(intent.fwau, IntentReject::RateLimited);
        ...
        if intent.verb == Verb::Move && intent.payload.len() >= 8 {
            ...
            if dx.abs() > 1.05 || dy.abs() > 1.05 {
                self.netcode
                    .record_reject(intent.fwau, IntentReject::SpeedHack);
            ...
            if let Some(rec) = self.world.get_mut(entity) {
                ...
                if let Some(binding) = &rec.fwau_binding {
                    ...
                    if dist > max_step {
                        ...
                        rec.velocity.linear = Vec3::ZERO;
                        self.netcode
                            .record_reject(binding.fwau_id, IntentReject::SpeedHack);
                    } else {
                        self.player_last_pos
                            .insert(binding.fwau_id, rec.transform.position);
                    }
                }
    ...
    pub fn iuoc_for_fwau(&self, fwau: FwauId) -> Option<IuocId> {
        self.find_avatar_for_fwau(fwau)
    ...
    pub fn build_replication(&self) -> ReplicationPacket {
        let entities: Vec<EntitySnapshot> = self
            ...
            .filter_map(|&e| {
                self.world
                    .get(e)
                    .filter(|r| r.dirty || r.sleep.awake)
                    .map(|rec| EntitySnapshot {
                        entity: rec.entity,
                        name: rec.name.clone(),
                        x: rec.transform.position.x,
                        y: rec.transform.position.y,
