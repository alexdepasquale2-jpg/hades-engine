...
impl SoulArchive {
    pub fn open(path: impl AsRef<Path>) -> Result<Self, PersistError> {
        ...
        conn.execute_batch(SCHEMA).map_err(PersistError::Sqlite)?;
        Ok(Self {
            conn,
    ...
    pub fn load_souls(&self) -> Result<Vec<IUOC>, PersistError> {
        ...
        while let Some(row) = rows.next().map_err(PersistError::Sqlite)? {
            ...
            let bound_fwau = bound_raw.map(|s| parse_u128(&s).map(FwauId)).transpose()?;
    ...
    pub fn load_packets(&self) -> Result<HashMap<IuocId, Vec<ExperiencePacket>>, PersistError> {
        ...
        while let Some(row) = rows.next().map_err(PersistError::Sqlite)? {
            ...
            let packet = ExperiencePacket {
                ...
                avatar: crate::types::AvatarId(
                    row.get::<_, i64>(2).map_err(PersistError::Sqlite)? as u64
                ),
                frame: crate::types::FrameId(
                    row.get::<_, i32>(3).map_err(PersistError::Sqlite)? as u32
                ),
                bound_at: crate::types::Tick(
                    row.get::<_, i64>(4).map_err(PersistError::Sqlite)? as u64
                ),
                unbound_at: crate::types::Tick(
                    row.get::<_, i64>(5).map_err(PersistError::Sqlite)? as u64
                ),
...
fn parse_u128(s: &str) -> Result<u128, PersistError> {
    s.parse::<u128>()
        .map_err(|_| PersistError::ParseId(s.to_string()))
}
