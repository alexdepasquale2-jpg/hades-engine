...
impl OpsSnapshot {
  pub fn from_aum(aum: &AumCore, active_sessions: usize) -> Self {
    ...
    if let Some(stats) = &archive {
      checks.push(HealthCheck {
        name: "archive".into(),
        ok: true,
        detail: format!("{} souls, {} packets at {}", stats.souls, stats.packets, stats.path),
      });
    } else {
      checks.push(HealthCheck {
        name: "archive".into(),
    ...
    let guardrail_ok = frames
      .iter()
      .all(|f| f.guardrails.as_ref().map(|g| g.within_budget).unwrap_or(true));
    checks.push(HealthCheck {
  ...
  pub fn prometheus_lines(&self) -> String {
    ...
    append_metric(&mut out, "tbc_info", &format!("version=\"{}\"", self.version), 1.0);
    append_metric(
      &mut out,
      "tbc_ready",
      "",
    ...
    for frame in &self.frames {
      ...
      if let Some(g) = &frame.guardrails {
        append_metric(&mut out, "tbc_guardrail_stalls", &labels, g.stall_count as f64);
        append_metric(
          &mut out,
