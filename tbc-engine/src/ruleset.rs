...
impl RulesetRegistry {
    ...
    pub fn load_dir(path: &std::path::Path) -> Result<Self, RulesetLoadError> {
        ...
        for entry in std::fs::read_dir(path).map_err(RulesetLoadError::Io)? {
            ...
            let ruleset = Ruleset::from_json(&text)
                .map_err(|e| RulesetLoadError::Json(p.display().to_string(), e))?;
            reg.insert(ruleset);
        }
    ...
    pub fn boot_defaults() -> Self {
        let path = std::env::var("TBC_RULESETS_PATH")
            .unwrap_or_else(|_| format!("{}/../rulesets", env!("CARGO_MANIFEST_DIR")));
