...
impl Frame {
  pub fn try_attack(
    ...
    let (target_entity, target_name) = if let Some(t) = target {
      let name = self.world.get(t).map(|r| r.name.clone()).unwrap_or_default();
      (t, name)
    } else {
    ...
    if let Some(rec) = self.world.get_mut(player_entity) {
      let avatar = rec.avatar.as_mut().ok_or_else(|| "no avatar state".to_string());
      if avatar.is_err() {
...
mod tests {
  ...
  fn attack_reduces_ai_hp() {
    ...
    let fwau = frame
      ...
      .unwrap();
    let ai = frame.world.spawn_ai_guy("AI-Guy-0", Vec3::new(2.0, 0.0, 0.0), 1);
  ...
  fn interact_grants_unique_item() {
    ...
    let fwau = frame
      .bind_player(&mut reg, iuoc, Vec3::new(0.0, 0.0, 0.0))
      .unwrap();
    frame.world.spawn_ai_guy("AI-Guy-0", Vec3::new(2.0, 0.0, 0.0), 1);
    let mut ledger = EntropyLedger::new();
